...
class LM(object):
    ...
    def generate(self, input_str: str,
        ...
        top_k = top_k if top_k is not None else getattr(self.model.config, 'top_k', 50)
        top_p = top_p if top_p is not None else getattr(self.model.config, 'top_p', 1.0)
        temperature = temperature if temperature is not None else getattr(self.model.config, 'temperature', 1.0)
